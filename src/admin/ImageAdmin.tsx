    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ marginBottom: 4 }}>
        <p style={{ fontSize: 13, color: '#737373', margin: 0 }}>
          Pacotes marcados como 🔥 <strong style={{ color: '#f7ad40' }}>Em Alta</strong>. Estes aparecem no carrossel principal do site.
        </p>
      </div>
...
function PackageCard({ pkg, index, total, categories, otherSlugs = [], allPackages, onUpdate, onRemove, onReorder, onSetTrending, onSetHidden, onDuplicate, onSaved, isOpen, onToggle, dragHandleProps }: {
  ...
  /* Pacote de OUTRO portal (integração): conteúdo é somente leitura aqui —
     este portal controla apenas exibição, Em Alta e ordem. */
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }} onClick={handleToggle}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {hasEdited && (
              <span style={{ fontSize: 10, background: '#3d2800', color: '#f7ad40', border: '1px solid #7a4f00', padding: '2px 8px', borderRadius: 12, fontWeight: 700 }}>● Não salvo</span>
            )}
          </div>
          <div style={{ fontSize: 12, color: '#737373', marginTop: 2 }}>{pkg.date} · {pkg.loc} · {pkg.currency || 'BRL'} {pkg.price}</div>
        </div>
      ...
      {isOpen && isExternal && (
        <div style={{ padding: '0 16px 20px', borderTop: '1px solid #333333' }}>
          <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {trendMsg && (
              <div style={{ background: trendMsg.startsWith('🔥') ? '#0d3320' : '#1a2030', border: '1px solid #333333', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: trendMsg.startsWith('🔥') ? '#4ade80' : '#f7ad40', fontWeight: 600 }}>
                {trendMsg}
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              <div style={{ background: '#001a33', border: '1px solid #333333', borderRadius: 10, padding: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              ...
              <div style={{ background: '#001a33', border: '1px solid #333333', borderRadius: 10, padding: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: pkg.isTrending ? '#f7ad40' : '#737373', display: 'flex', alignItems: 'center', gap: 4 }}><Flame size={12} /> Em Alta neste portal</span>
                <button type="button" onClick={handleTrendToggle}
                  style={{ width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', position: 'relative', transition: 'background .2s', background: pkg.isTrending ? '#f7ad40' : '#333333' }}>
        ...
        <div style={{ padding: '0 16px 20px', borderTop: '1px solid #333333' }}>
          <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {pkg.sharedId != null && onSetHidden && (
              <div style={{ background: '#001a33', border: '1px solid #333333', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            ...
            <AdminSection title="Informações Básicas" icon={Tag}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
                ...
                <Field label="Parcelas (ex: 10)" icon={<DollarSign size={11} />} value={local.installments || ''} onChange={v => set({ installments: v })} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, gridColumn: '1 / -1' }}>
                  <label style={{ fontSize: 11, color: '#737373', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <button type="button" onClick={() => set({ slug: uniqueSlug(slugify(local.title), otherSlugs) })}
                      style={{ background: '#002042', border: '1px solid #333', color: '#aaa', padding: '2px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 10, fontWeight: 700 }}>
                      Gerar do título
                    </button>
                  </label>
                  <input
                    ...
                    placeholder="ex: copa-do-mundo-2026 (gerado automaticamente ao editar o título)"
                    style={{ background: '#001529', border: '1px solid #333333', borderRadius: 7, color: '#e8edf2', fontSize: 13, padding: '9px 12px', outline: 'none' }}
                  />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, paddingBottom: 2 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: pkg.isTrending ? '#f7ad40' : '#333333',
                    }}
            ...
            <div style={{
              position: 'sticky', bottom: 0, marginTop: 8, padding: '12px 16px', background: '#002042',
              border: `1px solid ${hasEdited ? '#f7ad40' : '#333333'}`, borderRadius: 12,
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
            }}>
              <span style={{ fontSize: 12, color: hasEdited ? '#f7ad40' : '#737373', fontWeight: 600 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: hasEdited ? 'linear-gradient(135deg, #f7ad40, #d45f1a)' : '#333333',
                  color: hasEdited ? '#002042' : '#737373', border: 'none', borderRadius: 8,
...
function PackagesTab() {
  const { packages, categories, updatePackage, addPackage, duplicatePackage, removePackage, reorderPackage, setPackageTrending, setPackageHidden } = useContentConfig();
